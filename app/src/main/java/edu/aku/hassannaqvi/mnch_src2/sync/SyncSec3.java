...
public class SyncSec3 extends AsyncTask<Void, Void, String> {
    ...
    protected String doInBackground(Void... params) {
        try {
            return downloadUrl(SRCApp._HOST_URL + "/src2/api/sec3.php");
        } catch (IOException e) {
    ...
    private String downloadUrl(String myurl) throws IOException {
        ...
        Collection<Sec3Contract> Forms;
        //if (flag) {
        Forms = db.getUnsyncedSec3();
        /*} else {
            Forms = db.getFormsSg();
        }*/
...
//            pd.setMessage("Total Forms: " );
    ...
    protected void onPostExecute(String result) {
        ...
        try {
            ...
            Toast.makeText(mContext, sSynced + " Sec 3's synced." + String.valueOf(json.length() - sSynced) + " Errors: " + sSyncedError, Toast.LENGTH_SHORT).show();

            pd.setMessage(sSynced + " Sec 3's synced." + String.valueOf(json.length() - sSynced) + " Errors: " + sSyncedError);
            pd.setTitle("Done uploading Sec 3's data");
            pd.show();
        } catch (JSONException e) {
            e.printStackTrace();
