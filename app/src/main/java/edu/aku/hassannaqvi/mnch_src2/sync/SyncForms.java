...
public class SyncForms extends AsyncTask<Void, Void, String> {
    ...
    protected String doInBackground(Void... params) {
        try {
            return downloadUrl(SRCApp._HOST_URL + "/src2/api/forms.php");
        } catch (IOException e) {
...
//            pd.setMessage("Total Forms: " );
    ...
    protected void onPostExecute(String result) {
        ...
        try {
            ...
            Toast.makeText(mContext, sSynced + " Forms's synced." + String.valueOf(json.length() - sSynced) + " Errors: " + sSyncedError, Toast.LENGTH_SHORT).show();

            pd.setMessage(sSynced + " Form's synced." + String.valueOf(json.length() - sSynced) + " Errors: " + sSyncedError);
            pd.setTitle("Done uploading Form's data");
            pd.show();
        } catch (JSONException e) {
            e.printStackTrace();
